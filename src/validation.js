...
export function validateHierarchyWildcard(value, fieldName) {
  ...
  if (gtIndex !== -1 && gtIndex !== tokens.length - 1) {
    throw new Error(
      `${fieldName} invalid: ">" can only be at the end of a topic`,
    );
  }
}
...
export function validateCommandName(value) {
  ...
  if (!IDENT_REGEX.test(value)) {
    throw new Error(
      "command name contains invalid characters. Allowed: a-z, A-Z, 0-9, _, -",
    );
  }
}
...
export function validateRpcName(value) {
  ...
  if (!IDENT_REGEX.test(value)) {
    throw new Error(
      "rpc name contains invalid characters. Allowed: a-z, A-Z, 0-9, _, -",
    );
  }
}
...
export function validateTelemetryMetric(value) {
  ...
  if (!IDENT_REGEX.test(value)) {
    throw new Error(
      "metric contains invalid characters. Allowed: a-z, A-Z, 0-9, _, -",
    );
  }
}
