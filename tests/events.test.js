  ...
  describe("stream", () => {
    it("creates consumer with wildcard subject", async () => {
      ...
      expect(opts.filter_subjects).toBe(
        "test_org_123.production.events.*.door_opened",
      );
    });
    ...
    it("throws on invalid name", async () => {
      ...
      const em = new EventManager(ctx);
      await expect(
        em.stream({ name: "a.b", callback: vi.fn() }),
    ...
    it("throws on non-function callback", async () => {
      ...
      const em = new EventManager(ctx);
      await expect(
        em.stream({ name: "test", callback: "not_fn" }),
    ...
    it("throws when not connected", async () => {
      ...
      const em = new EventManager(ctx);
      await expect(
        em.stream({ name: "test", callback: vi.fn() }),
