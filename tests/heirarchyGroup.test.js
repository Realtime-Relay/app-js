...
function makeCtx(consumer) {
  const ctx = createMockContext({
    consumer,
    responses: {
      "api.iot.cohort.test_org_123.heirarchy.create": {
      ...
      "api.iot.cohort.test_org_123.heirarchy.update": {
        status: "HIERARCHY_GROUP_UPDATE_SUCCESS",
        data: { id: "hg1", name: "building_a", heirarchy: "campus.building_a" },
      },
      "api.iot.cohort.test_org_123.heirarchy.delete": {
        status: "HEIRARCHY_GROUP_DELETE_SUCCESS",
      },
      "api.iot.cohort.test_org_123.heirarchy.list": {
        status: "HEIRARCHY_GROUP_LIST_SUCCESS",
        data: [
          { id: "hg1", name: "building_a", heirarchy: "campus.building_a" },
        ],
      },
      "api.iot.cohort.test_org_123.heirarchy.get": {
        status: "HEIRARCHY_GROUP_GET_SUCCESS",
        data: { id: "hg1", name: "building_a", heirarchy: "campus.building_a" },
      },
      "api.iot.cohort.test_org_123.heirarchy.device.list": {
        status: "HEIRARCHY_DEVICE_LIST_SUCCESS",
        data: [{ id: "dev_1" }],
      },
    },
  });
...
describe("HeirarchyGroupManager", () => {
  describe("create", () => {
    ...
    it("validates hierarchy name allows dots", async () => {
      ...
      await expect(
        hgm.create({ name: "a.b", heirarchy: "x.y", device_idents: [] }),
    ...
    it("rejects wildcards in hierarchy name on create", async () => {
      ...
      const hgm = new HeirarchyGroupManager(ctx);
      await expect(
        hgm.create({ name: "a", heirarchy: "x.*", device_idents: [] }),
  ...
  describe("stream", () => {
    it("creates consumer with correct subject", async () => {
      ...
      expect(streamCall[1].filter_subjects).toBe(
        "import.test_org_123.production.heirarchy.listen.*.campus.building_a",
      );
    });
    ...
    it("accepts wildcard hierarchy filter", async () => {
      ...
      await expect(
        group.stream({ heirarchy: "building_a.*", callback: vi.fn() }),
