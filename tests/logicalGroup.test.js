...
function makeCtx(consumer, responseOverrides = {}) {
  const ctx = createMockContext({
    consumer,
    responses: {
      "api.iot.cohort.test_org_123.logical.create": {
        status: "LOGICAL_GROUP_CREATE_SUCCESS",
        data: { id: "g_new", name: "floor_1" },
      },
      "api.iot.cohort.test_org_123.logical.update": {
        status: "LOGICAL_GROUP_UPDATE_SUCCESS",
        data: { id: "g1", name: "floor_1_updated" },
      },
      "api.iot.cohort.test_org_123.logical.delete": {
        status: "LOGICAL_GROUP_DELETE_SUCCESS",
      },
      "api.iot.cohort.test_org_123.logical.list": {
        status: "LOGICAL_GROUP_LIST_SUCCESS",
        data: [{ id: "g1", name: "floor_1" }],
      },
      "api.iot.cohort.test_org_123.logical.get": {
        status: "LOGICAL_GROUP_GET_SUCCESS",
        data: { id: "g1", name: "floor_1" },
      },
      "api.iot.cohort.test_org_123.logical.device.list": {
        status: "LOGICAL_DEVICE_LIST_SUCCESS",
        data: [{ id: "dev_1" }],
      },
      ...responseOverrides,
    },
...
describe("LogicalGroupManager", () => {
  describe("create", () => {
    ...
    it("throws on invalid name", async () => {
      ...
      const lgm = new LogicalGroupManager(ctx);
      await expect(
        lgm.create({ name: "a.b", tags: [], device_idents: [] }),
  ...
  describe("stream", () => {
    it("creates consumer with correct subject", async () => {
      ...
      expect(streamCall[1].filter_subjects).toBe(
        "import.test_org_123.production.group.listen.g1",
      );
    });
  });
