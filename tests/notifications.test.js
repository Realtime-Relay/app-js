...
function makeCtx(responseOverrides = {}) {
  return createMockContext({
    responses: {
      "api.iot.notification.test_org_123.create": {
        status: "NOTIFICATION_CREATE_SUCCESS",
        data: { name: "ops", type: "WEBHOOK" },
      },
      "api.iot.notification.test_org_123.update": {
        status: "NOTIFICATION_UPDATE_SUCCESS",
        data: { name: "ops" },
      },
      "api.iot.notification.test_org_123.delete": {
        status: "NOTIFICATION_DELETE_SUCCESS",
      },
      "api.iot.notification.test_org_123.list": {
        status: "NOTIFICATION_LIST_SUCCESS",
        data: [{ name: "ops" }],
      },
      "api.iot.notification.test_org_123.get": {
        status: "NOTIFICATION_FETCH_SUCCESS",
        data: { name: "ops", type: "WEBHOOK" },
      },
      ...responseOverrides,
    },
...
describe("NotificationManager", () => {
  describe("create", () => {
    ...
    it("throws on invalid type", async () => {
      ...
      const nm = new NotificationManager(ctx);
      await expect(
        nm.create({ name: "x", type: "SMS", config: {} }),
    ...
    it("throws on missing WEBHOOK endpoint", async () => {
      ...
      const nm = new NotificationManager(ctx);
      await expect(
        nm.create({ name: "x", type: "WEBHOOK", config: {} }),
