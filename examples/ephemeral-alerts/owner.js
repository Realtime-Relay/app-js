...
ephAlert.setEvaluator((data) => {
    ...
    if (temp == null || humidity == null) {
        console.log(`  [eval] Waiting for data... temp=${temp ?? '?'} humidity=${humidity ?? '?'}`);
        return false;
    }
...
console.log(`Monitoring ${DEVICE_IDENT}: temperature > ${TEMP_THRESHOLD}°C AND humidity > ${HUMIDITY_THRESHOLD}%`);
