...
// ── Get the ephemeral alert (created by owner.js) ────────────

const ALERT_NAME = 'ephemeral_multi_metric';

const ephAlert = await app.alert.get(ALERT_NAME);

if (!ephAlert) {
    console.error(`Alert "${ALERT_NAME}" not found. Run owner.js first.`);
...
// This subscribes to import.{orgID}.{env}.alerts.listen.{rule_id}.*
// and receives fire/resolved/ack/ack_all events published by the owner.
...
console.log('Listening for alert events (listener mode).\n');
